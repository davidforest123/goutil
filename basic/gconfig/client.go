...
func NewClient(customConfigDir string) (*Client, error) {
	...
	_, files, err := gfs.WalkDirTopLevel(cfgDir)
	if err != nil {
		return nil, err
	}
	for _, filename := range files {
		...
		filename = gfs.PathJoin(cfgDir, filename)
...
func (c *Client) cryptFn(method, key string, val interface{}) (newVal interface{}, modified bool, err error) {
	if !gstring.EndsWith(key, "EncryptMe") { // Don't encrypt/decrypt key doesn't end with "EncryptMe".
...
func (c *Client) Load(configFileName string, v interface{}, allowEmpty bool) error {
	...
	if ok && cfgVal != "" {
		if gstring.EndsWith(strings.ToLower(configFileName), ".json") {
			cfgValCopy := cfgVal
		...
		} else if gstring.EndsWith(strings.ToLower(configFileName), ".csv") {
