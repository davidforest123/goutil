...
func SelfDir() (string, error) {
	...
	res := filepath.Dir(p) // 不仅是末尾的文件名被删了，最终返回的目录路径最后一位的斜杠也被删了

	// 返回的路径建议保留斜杠，不包含斜杠的路径有时会被认为是文件路径而非目录路径，比如在filepath.Join()的第一个参数中，不包含斜杠的路径会被认为是文件路径进而对其进行裁剪。
	if len(res) >= 2 && res != ".." && res[len(res)-1] != os.PathListSeparator {
