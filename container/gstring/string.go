...
// RemoveHeadSubStr deletes all `substr` from head of `s`.
func RemoveHeadSubStr(s, substr string) string {
	result := s
	for StartsWith(result, substr) {
		result = RemoveHead(result, len(substr))
	}
	return result
}
...
// RemoveTailSubStr deletes all `substr` from tail of `s`.
func RemoveTailSubStr(s, substr string) string {
	result := s
	for EndsWith(result, substr) {
		result = RemoveTail(result, len(substr))
	}
	return result
}
...
func StartsWith(s, find string) bool {
	if len(s) == 0 || len(find) == 0 {
		return false
	}

	return strings.Index(s, find) == 0
}

// StartsWithAny check if `s` contains anyone of the `finds` strings array.
func StartsWithAny(s string, finds ...string) bool {
	if len(s) == 0 || len(finds) == 0 {
		return false
	}
...
func EndsWith(s, toFind string) bool {
	...
	// if don't add pos >= 0, there will be a bug if EndsWith("astring", "*astring")
