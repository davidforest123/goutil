...
func ParseString(speed string) (Speed, error) {
	...
	if gstring.EndsWith(strings.ToLower(unitStr), "/s") || gstring.EndsWith(strings.ToLower(unitStr), "ps") {
...
func MustParseString(speed string) Speed {
	result, err := ParseString(speed)
	if err != nil {
		panic(err)
	}
	return result
