...
func Parse(s string) (*Interval, error) {
	...
	if !gstring.StartsWith(ss[0], "[") && !gstring.StartsWith(ss[0], "(") {
		return nil, defErr
	}
	if !gstring.EndsWith(ss[1], "]") && !gstring.EndsWith(ss[1], ")") {
		return nil, defErr
	}
	...
	if leftNum == "-∞" {
		...
		min, err := gnum.NewDecimalFromString(leftNum)
		if err != nil {
			return nil, defErr
		}
		res.Min = &min
	...
	if rightNum == "+∞" {
		...
		max, err := gnum.NewDecimalFromString(rightNum)
		if err != nil {
			return nil, defErr
		}
		res.Max = &max
	...
	res.IncludeMin = res.Min != nil && gstring.StartsWith(s, "[")
	res.IncludeMax = res.Max != nil && gstring.EndsWith(s, "]")
