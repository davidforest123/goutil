...
/*
   ...
   func ParseUrlString(u string) (*UrlChunk, error) {
   	...
   	if ss := strings.Split(uphppp, "@"); len(ss) == 2 {
   		...
   		if len(ss) == 1 {
   			if gstring.StartsWith(usr_pwd, ":") {
   				r.Password = usr_pwd
   			}
   			if gstring.EndsWith(usr_pwd, ":") {
   				r.User = usr_pwd
   			}
   		}
   		if len(ss) == 2 {
...
func ParseUrl(urlStr string) (*AddrSlice, error) {
	...
	if gstring.StartsWith(strings.ToLower(urlStr), strings.ToLower(u.Scheme)+"://") {
		s.Scheme = u.Scheme
	}
	if u.User != nil {
