...
func TestReuseListenPacketWildcardAddress(t *testing.T) {
	pc, err := ReuseListenPacket("udp", ":0")
	require.NoError(t, err)
	pc.Close()
}

func TestErrorWhenReuseDialUnresolvable(t *testing.T) {
	_, err := ReuseDial("asd", "127.0.0.1:1234", "127.0.0.1:1234")
	require.ErrorIs(t, err, net.UnknownNetworkError("asd"))
	_, err = ReuseDial("tcp", "a.b.c.d:1234", "a.b.c.d:1235")
	require.Error(t, err)
}
