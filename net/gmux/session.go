...
func (s *Session) GetNoDataTimeout() time.Duration {
	s.noDataTimeoutMu.RLock()
	defer s.noDataTimeoutMu.RUnlock()
	return s.noDataTimeout
}
...
func (s *Session) SetNoDataTimeout(noDataTimeout time.Duration) error {
	if noDataTimeout <= 0 {
		return gerrors.New("invalid noDataTimeout %s", noDataTimeout.String())
	}

	s.noDataTimeoutMu.Lock()
	s.noDataTimeout = noDataTimeout
	s.noDataTimeoutMu.Unlock()
	s.noDataTicker.Reset(noDataTimeout)
	return nil
}
...
func (s *Session) OpenStream(streamName string) (*Stream, error) {
	...
	if len(streamName) > 0 {
		synFrame.data = []byte(streamName) // set stream name // TODO 改成JSON，加上NoDataTimeout
	}
