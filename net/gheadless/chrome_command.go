...
/**
copyright: https://github.com/sensepost/gowitness/blob/master/chrome/proxy.go
*/
...
func (chrome *Chrome) checkVersion(lowestVersion string) bool {
	...
	if err != nil {
		//log.WithFields(log.Fields{"chrome-path": chrome.Path, "err": err}).Error("An error occurred while trying to get the Chrome version")
		return false
	}
	...
	if gover.Compare(version, lowestVersion, "<") {
		//log.WithFields(log.Fields{"chrome-path": chrome.Path, "chromeversion": version}).Warn("Chrome version is older than v" + lowestVersion)
		return false
	}
...
func (chrome *Chrome) SetScreenshotPath(p string) error {
	...
	if _, err := os.Stat(p); os.IsNotExist(err) {
		return gerrors.New("Destination path does not exist")
	}
...
func (chrome *Chrome) ScreenshotURL(targetURL *url.URL, destination string) error {
	...
	//log.WithFields(log.Fields{"url": targetURL, "full-destination": destination}).Debug("Full path to screenshot save using Chrome")
	...
	if err := cmd.Wait(); err != nil {
		...
		if ctx.Err() == context.DeadlineExceeded {
			//log.WithFields(log.Fields{"url": targetURL, "destination": destination, "err": err}).Error("Timeout reached while waiting for screenshot to finish")
			return err
		}

		//log.WithFields(log.Fields{"url": targetURL, "destination": destination, "err": err}).Error("Screenshot failed")

		return err
	}

	//log.WithFields(log.Fields{"url": targetURL, "destination": destination, "duration": time.Since(startTime),}).Info("Screenshot taken")
