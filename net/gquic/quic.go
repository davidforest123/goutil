...
func Dial(raddr string) (*QuicConn, error) {
	sess, err := quic.DialAddr(context.Background(), raddr, &tls.Config{InsecureSkipVerify: true, NextProtos: []string{alpn}}, nil)
	if err != nil {
		return nil, err
	}
...
// generateTLSConfig setups a bare-bones TLS config for the server
