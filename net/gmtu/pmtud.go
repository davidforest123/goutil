...
func Pmtud(addr string) (int, error) {
	...
	max := 65520
	min := 0
	good, rec, err := check(ip.String(), 1500-overhead)
	if err != nil {
		return 0, err
	}
	if good {
		min = 1500
		good, rec, err = check(ip.String(), 1501-overhead)
		if err != nil {
			return 0, err
		}
		if !good {
	...
	if min == 1501 {
		good, rec, err = check(ip.String(), 65520-overhead)
		if err != nil {
			return 0, err
		}
		if good {
	...
	for rec != 0 {
		good, nRec, err := check(ip.String(), rec-overhead)
		if err != nil {
			return 0, err
		}
		if good {
	...
	for max != min {
		...
		good, _, err := check(ip.String(), pmtu)
		if err != nil {
			return 0, err
		}
		if good {
