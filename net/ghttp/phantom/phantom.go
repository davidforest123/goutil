...
func (self *Phantom) Download(req Request) (resp *http.Response, err error) {
	...
	return nil, gerrors.New("Download error")
}
...
/*
func PhantomGet(url, proxy, userAgent string, timeout time.Duration) (resp http.Response, err error) {
	ph := NewPhantom()
	ph.SetProxy(proxy)
	ph.Download(http.NewRequest("GET", url))
}*/
