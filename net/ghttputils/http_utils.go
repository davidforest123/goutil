...
func NewHttpRequestWithFasthttp(client *http.Client, reqMethod, reqUrl, postData string, headers map[string]string) ([]byte, error) {
	glog.Debgf("use fasthttp client")
	transport := client.Transport
	if transport != nil {
		if proxy, err := transport.(*http.Transport).Proxy(nil); err == nil && proxy != nil {
			proxyUrl := proxy.String()
			glog.Debgf("proxy url: ", proxyUrl)
			if proxy.Scheme != "socks5" {
				glog.Errof("fasthttp only support the socks5 proxy")
			} else {
	...
	if resp.StatusCode() != 200 {
		return nil, gerrors.New(fmt.Sprintf("HttpStatusCode:%d ,Desc:%s", resp.StatusCode(), string(resp.Body())))
	}
...
func NewHttpRequest(client *http.Client, reqType string, reqUrl string, postData string, requstHeaders map[string]string) ([]byte, error) {
	glog.Debgf("[%s] request url: %s", reqType, reqUrl)
	...
	if resp.StatusCode != 200 {
		return nil, gerrors.New(fmt.Sprintf("HttpStatusCode:%d ,Desc:%s", resp.StatusCode, string(bodyData)))
	}
