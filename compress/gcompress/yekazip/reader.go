...
// OpenReaderWithFile will open the Zip file specified by name and return a ReadCloser.
func OpenReaderWithFile(f *os.File) (*ReadCloser, error) {
	fi, err := f.Stat()
	if err != nil {
		f.Close()
		return nil, err
...
func (f *File) Open() (rc io.ReadCloser, err error) {
	...
	dcomp := decompressor(uint16(f.Method))
	if dcomp == nil {
		err = ErrAlgorithm
		return
	}
