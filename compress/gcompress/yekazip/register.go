...
var (
	...
	compressors = map[uint16]Compressor{
		uint16(LevelStore):   func(w io.Writer) (io.WriteCloser, error) { return &nopCloser{w}, nil },
		uint16(LevelDeflate): func(w io.Writer) (io.WriteCloser, error) { return newFlateWriter(w), nil },
	}

	decompressors = map[uint16]Decompressor{
		uint16(LevelStore):   ioutil.NopCloser,
		uint16(LevelDeflate): flate.NewReader,
	}
)
