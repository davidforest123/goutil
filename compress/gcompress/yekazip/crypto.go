...
func (h *FileHeader) writeWinZipExtra() {
	...
	eb.uint16(uint16(h.Method)) // original compression method
...
func (w *Writer) Encrypt(name string, password string, enc EncryptionMethod, level Level) (io.Writer, error) {
	fh := &FileHeader{
		Name:   name,
		Method: level,
	}
